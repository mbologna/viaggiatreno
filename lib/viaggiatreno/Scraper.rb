...
class Scraper
  ...
  def initialize(trainNumber, train)
    @site_info_main = ViaggiatrenoURLs.SITE_INFO_MAIN.gsub(RegExpMatchInfo.STR_TRAIN_NUMBER_URL_REPLACE, trainNumber)
    @site_info_details = ViaggiatrenoURLs.SITE_INFO_DETAILS.gsub(RegExpMatchInfo.STR_TRAIN_NUMBER_URL_REPLACE, trainNumber)
    @train = train
  end
  ...
  def updateTrain()
    ...
    if @status =~ RegExpMatchInfo.REGEXP_STATE_NOT_STARTED
    ...
    elsif @status =~ RegExpMatchInfo.REGEXP_STATE_RUNNING or RegExpMatchInfo.REGEXP_STATE_FINISHED
      ...
      if @status =~ RegExpMatchInfo.REGEXP_STATE_RUNNING
        ...
        @train.lastUpdate = @status.match(RegExpMatchInfo.REGEXP_STATE_RUNNING)[3].strip
  ...
  def updateTrainDetails()
    ...
    doc.xpath(XPathMatchInfo.XPATH_DETAILS_GENERIC).each do |x|
      ...
      x.xpath(XPathMatchInfo.XPATH_DETAILS_SCHEDULED_STOP_TIME).each do |scheduledArrivalTime|
        @scheduledArrivalTime = StringUtils.remove_newlines_tabs_and_spaces(scheduledArrivalTime).to_s
      end
      x.xpath(XPathMatchInfo.XPATH_DETAILS_ACTUAL_STOP_TIME).each do |actualArrivalTime|
        @actualArrivalTime = StringUtils.remove_newlines_tabs_and_spaces(actualArrivalTime).to_s
      end
      if x.attributes()['class'].to_s =~ RegExpMatchInfo.REGEXP_STOP_ALREADY_DONE
        t = TrainStop.new(@stationName, @scheduledArrivalTime, @actualArrivalTime, StopState.DONE)
      else
        t = TrainStop.new(@stationName, @scheduledArrivalTime, @actualArrivalTime, StopState.TODO)
      end
      @train.addStop(t)
    end
  end
